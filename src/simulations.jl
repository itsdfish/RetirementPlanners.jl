"""
    grid_search(parameters::AbstractDict, initialize; kwargs...) → adf, mdf
...
## Keywords
The following keywords modify the `grid_search` function:
...
## Example
A runnable example that uses `grid_search` is shown in [Schelling's segregation model](@ref).
There, we define
```julia
...
adf, _ = grid_search(parameters, initialize; adata, n = 3)
```
"""
function grid_search(
    ...
    @warn "Passing agent_step! and model_step! to grid_search is deprecated.
